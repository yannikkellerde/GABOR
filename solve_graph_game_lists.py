...
class PN_search():
    ...
    def set_pn_dn(self, n, is_prove_node):
        if n[PN] == 0 or n[DN] == 0:
            return
        if is_prove_node:
    ...
    def delete_node(self, n, ps, ch, is_prove_node):
        for p in ps:
        ...
        for _,c in ch:
            if len(c)>PARENTS:
                c[PARENTS].remove(n)
                if len(c[PARENTS])==0:
                    self.delete_node(c, [], c[CHILDREN].copy(),not is_prove_node)
    ...
    def update_anchestors(self, n, is_prove_node):
        #print(depth,n[HASH])
        old_pn = n[PN]
        old_dn = n[DN]
